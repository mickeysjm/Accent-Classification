...
for k = 1:N_seg
    ...
    % For logistic Regression, no need to export
...
% For logistic regression, this part is unnecessary because the original
% model to choose the best regularization parameter automatically.
% para = load('parameters.csv');

%% L2-regularized Logistic Regression Model for each segment
...
for k = 1:N_seg
    ...
    % first get the best regularization parameter C
    tmp_model = train(Y_training, sparse(X_training),option);
    C = tmp_model(1);
    % next train the model
    option2 = char(strcat({'-s 0 -c '},{num2str(C)}));
    model = train(Y_training, sparse(X_training),option2);
    [p,a,d] = predict(Y_testing, sparse(X_testing), model);
    PredictLabel(:,k) = p;
    Accuracy(1,k) = a(1);
end

%% Further Decision Based on Each Logistic Regression results
%
Count = zeros(67,3);
for i = 1:67
    for c = 1:3
...
%% Save Results
save('PredictLabel_PLP_LR.mat', 'PredictLabel');
save('Accuracy_PLP_LR.mat', 'Accuracy');
save('totalAccuarcy_PLP_LR.mat','totalAccuarcy');