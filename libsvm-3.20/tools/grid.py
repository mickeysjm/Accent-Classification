...
def redraw(db,best_param,gnuplot,options,tofile=False):
	...
	if tofile:
		gnuplot.write(b"set term jpg transparent small linewidth 2 medium enhanced\n")
...
svm_options : additional options for svm-train""")
	...
	try:
		result = find_parameters(dataset_pathname, options)
	...
	# # Add myself
	# filename = 'paramters.csv'
	# output = open(filename,'w')
	# for i in range(1,26):
	# 	print "------------------------------"
	# 	print i
	# 	print "------------------------------"
	# 	dataset_pathname = str(i)
	# 	try:
	# 		result = find_parameters(dataset_pathname, options)
	# 	except (IOError,ValueError) as e:
	# 		sys.stderr.write(str(e) + '\n')
	...
	# 	output.write(',')
	# 	output.write(str(result[1]['c']))
	# 	output.write(',')
	# 	output.write(str(result[1]['g']))
	# 	output.write('\n')
	# output.close()
